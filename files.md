...
- `prds/ai-judge-auth-gateway-report-facts.md`: Persist auth provider and Convex AI Gateway on AI results, detect community components, fall back to live `/hackathon.md` and live-site Convex signals when there is no repo log
...
- `convex/users.ts`: User management functions (queries, mutations, admin functions, mention search, recent vibers discovery, emoji theme preferences). `getMyUserDocument` returns a validator-safe subset of the current user row so extra schema fields cannot crash signed-in pages.
...
- `convex/spamCheckAnalysis.ts`: `analyzeSubmission` internal action for spam scans: deterministic checks in parallel (live URL GET with timeout, GitHub repo reachability and file count with empty-repo detection via optional `GITHUB_TOKEN`, video/LinkedIn/X link liveness), Firecrawl component scrape of the app URL, then an AI verdict (spam/suspicious/clean with confidence and reasons) through the shared Convex AI gateway caller in `convex/lib/llm.ts`, or a deterministic heuristic when the gateway call fails (result records provider "heuristic"); failures are recorded on the result row and never thrown upstream
- `convex/lib/llm.ts`: Shared LLM caller for the AI judge and the spam check: `callLlm` sends the system prompt and user message to the hosted Convex AI gateway via `@convex-dev/ai-sdk-provider` and the `ai` SDK's `generateText`, authenticated with the deployment's own service token so no provider API keys are stored; `DEFAULT_LLM_MODEL` is a gateway id in `provider/model` form and the provider recorded on result rows is the id prefix, keeping rows written before the gateway migration comparable
...
- `convex/emails/judgingGroupEmails.ts`: Judging group emails to judges or submission owners: send-status query (master switch, judging_group toggle, recipient count, rolling 24h usage against the 200-recipient daily cap), deduplicated judge and submission-owner recipient lists (owners plus the Hackathon Team Info team members on each submission, keyed per person as `owner:<storyId>` / `team:<storyId>:<email>` and deduplicated by lowercased address), per-send delivery stats grouped by sendId (delivered, opened, bounced, failed), send and test-send mutations gated by the judging.emails permission with recipientType (judges or submission_owners) selecting by recipientKeys (legacy storyIds still accepted), optional scheduled sends (runAt, cancellable, tracked in groupScheduledEmails), and the internal delivery action that renders per-recipient variables, matches each recipient to a VibeApps account by email (users `by_email` index) so the shared footer carries a working unsubscribe token, and routes through emails/resend.sendEmail with reply-to
...
- `src/components/admin/AdminDocs.tsx`: In-admin Docs tab with sidebar sections, Copy Markdown / Download .md on every page, and an External Judging Process guide for human judges (login, access codes, name rules, criteria, comments, notes, status, multi judge, filters, troubleshooting) plus existing docs for judging groups, criteria, submissions, judge flow, results, AI judge, agent API, spam check, delegated access, and environment variables
...
- `src/components/admin/judging/GroupSubmissionsTableSection.tsx`: Read-only roster table for one judging group (View submissions section) built on TanStack Table v9 with sorting and pagination features; columns are submission title plus slug, submitted date, tag pills with a +N overflow, submitter name and profile link, and LinkedIn, GitHub, X, and site links, each with a copy-to-clipboard button and a new-tab link; one search box filters title, slug, submitter, and tag names, page size is 25/50/100, and no mutation is wired into the section
...
- `src/components/admin/judging/GroupLinksSection.tsx`: Realtime links ledger for one judging group: every shareable URL (judging interface, results, custom submission page, plus AI results and agent API endpoints only while the AI judge is enabled) with lock/globe/key icons, live password-set status, copy and open actions, access notes, submit-page and score-scale hints, plus copy-all-as-markdown and .md file download that include each locked link's password as a one-paste event kit. Also lists the participant join link and renders the group's QR code inline when the custom submission page is enabled
- `src/components/admin/judging/GroupJoinQrCard.tsx`: Inline QR code card for a judging group's participant join link, shown in the Shareable links section. Renders the code client side from the live join URL with `qrcode.react` (nothing stored, so it never goes stale when the slug changes), pinned to black on white so it still scans in dark mode and in print. Actions: copy link, download a print-resolution PNG from an offscreen 1024px canvas, download a vector SVG, and preview the join page
...
- `src/components/admin/AIJudgeResults.tsx`: Admin AI Judge results view with Results, Stats, Convex Recap, and Hackathon Report tabs: Run AI Review with live progress counts, ranked per-criterion scores and reasoning, verified Convex facts, used vs installed components, auth provider and AI Gateway badges, source and URL liveness badges, frontend hosting, hackathon event and discrepancy indicators, inline score editing, retries, and closed per-submission brief panels with copy and `.md` save actions. Stats summarize Convex usage, auth providers, AI Gateway, components, and score distribution. The gated Convex Recap generates a no-PII markdown handoff for every completed submission, supports copy and `.md` export, and can include an optional persisted AI cohort summary with stale-result detection. The existing Hackathon Report keeps the organizer report with rankings, links, teams, participation, auth, and AI Gateway
...
- `src/components/ui/dialog.tsx`: Modal dialog component using the shared compact corner treatment
...
- `src/pages/SignInPage.tsx`: User sign-in page; honors a sanitized `redirect_url` query param as Clerk's `forceRedirectUrl` and carries it across the switch to sign-up, so QR and shared judging links return the user to the form they came from
...
- `src/lib/redirectPath.ts`: Post-auth return-destination helpers. `sanitizeRedirectPath` accepts only rooted same-origin paths (rejecting protocol-relative, absolute, backslash, and control-character values) so a crafted `redirect_url` cannot bounce a freshly signed-in user off-site; `authUrlWithReturn` builds `/sign-in` and `/sign-up` URLs that carry the destination
