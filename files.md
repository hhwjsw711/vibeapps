...
- `src/components/admin/AIJudgeResults.tsx`: Admin AI Judge results view with Results, Stats, Convex Recap, and Hackathon Report tabs: Run AI Review with live progress counts, ranked per-criterion scores and reasoning, verified Convex facts, used vs installed components, source and URL liveness badges, frontend hosting, hackathon event and discrepancy indicators, inline score editing, retries, and closed per-submission brief panels with copy and `.md` save actions. Stats summarize Convex usage and score distribution. The gated Convex Recap generates a no-PII markdown handoff for every completed submission, supports copy and `.md` export, and can include an optional persisted AI cohort summary with stale-result detection. The existing Hackathon Report keeps the organizer report with rankings, links, teams, and participation
