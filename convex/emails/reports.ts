    ...
    for (const adminUserId of args.adminUserIds) {
      ...
      const emailTemplate = await ctx.runQuery(
        internal.emails.templates.generateReportNotificationEmail,
        {
          ...
          storyUrl: `https://vibe.isllm.com/s/${story.slug || args.storyId}`,
          storySlug: story.slug,
          reportReason: report.reason,
          reportTimestamp: report._creationTime,
          dashboardUrl: `https://vibe.isllm.com/admin?tab=reports`,
          unsubscribeToken,
        },
      );
    ...
    for (const adminUserId of args.adminUserIds) {
      ...
      const emailTemplate = await ctx.runQuery(
        internal.emails.templates.generateAdminUserReportEmail,
        {
          ...
          dashboardUrl: `https://vibe.isllm.com/admin?tab=users&subtab=user-reports`,
          profileUrl: `https://vibe.isllm.com/${reportedUser.username || reportedUser._id}`,
          unsubscribeToken,
        },
      );
