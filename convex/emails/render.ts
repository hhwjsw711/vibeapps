...
export function emailPreferencesUrl(opts: {
  ...
  if (opts.username) {
    return `https://vibe.isllm.com/${opts.username}#email-preferences`;
  }
  if (opts.userId) {
    return "https://vibe.isllm.com/set-username";
  }
  return (
    "https://vibe.isllm.com/sign-in?redirect_url=" +
...
export function standardEmailFooter(opts: EmailFooterOpts = {}): string {
  ...
  const unsubscribeLink = opts.unsubscribeToken
    ? ` | <a href="https://vibe.isllm.com/api/unsubscribe?token=${opts.unsubscribeToken}" style="color: #666; font-size: 12px;">Unsubscribe</a>`
    : "";
  return `
